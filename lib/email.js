...
//   return content;
// }
export async function sendConfirmationEmail(email, token, password) {
    ...
    const html = await render(_jsx(ConfirmationEmail, { confirmationLink: confirmationLink, currentYear: currentYear, password: password }));
    if (!LOGO_BASE64) {
...
export async function sendAdminNotification(email) {
    ...
    // });
    const html = await render(_jsx(AdminNotificationEmail, { userEmail: email, currentYear: new Date().getFullYear().toString(), baseURL: baseUrl }));
    const mailOptions = {
        ...getBaseMailOptions(),
...
export async function sendFeedbackEmail(email) {
    const currentYear = new Date().getFullYear().toString();
    const html = await render(_jsx(FeedbackEmail, { currentYear: currentYear, useCid: true }));
    const mailOptions = {
        ...getBaseMailOptions(),
        to: email,
...
export async function sendSupportUsEmail(email) {
    ...
    const html = await render(_jsx(SupportUsEmail, { currentYear: currentYear, pixKey: "vaguinhas@vaguinhas.com.br", useCid: true }));
...
export async function sendFavouriteOnGithubEmail(email) {
    const html = await render(_jsx(FavoriteGithubEmail, { currentYear: new Date().getFullYear().toString() }));
    if (!LOGO_BASE64) {
        throw new Error("VAGUINHAS_LOGO is not defined");
    }
    const mailOptions = {
...
export async function sendConfirmEmailReminder(email, token) {
    const currentYear = new Date().getFullYear().toString();
    // Pre-render email HTML
    const html = await render(_jsx(ConfirmEmailReminder, { confirmationLink: `${process.env.NEXTAUTH_URL}/confirm-email?token=${token}`, currentYear: currentYear }));
    // if (!LOGO_BASE64) {
    //   throw new Error("VAGUINHAS_LOGO is not defined");
    // }
    const mailOptions = {
...
export async function sendProblemsEmail(email) {
    const currentYear = new Date().getFullYear().toString();
    // Renderiza o componente React para HTML
    const html = await render(_jsx(ProblemsEmail, { currentYear: currentYear, useCid: true }));
    const mailOptions = {
        ...getBaseMailOptions(),
        to: email,
...
export async function sendNewUpdateEmail(email) {
    const currentYear = new Date().getFullYear().toString();
    // Renderiza o componente React para HTML
    const html = await render(_jsx(NewUpdateEmail, { currentYear: currentYear, useCid: true }));
    const mailOptions = {
        ...getBaseMailOptions(),
        to: email,
...
export async function sendPasswordResetEmail(email, token) {
    ...
    const html = await render(_jsx(PasswordResetEmail, { resetLink: resetLink }));
    if (!LOGO_BASE64) {
        throw new Error("VAGUINHAS_LOGO is not defined");
    }
    const mailOptions = {
